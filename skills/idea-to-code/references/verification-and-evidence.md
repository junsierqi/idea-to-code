...
Evidence closeout: use `evidence closeout --root "$(pwd)" --slug <slug>` as a read-only pre-handoff evidence summary. It reports current `verify` status, open verification problems, latest milestone, delegation evidence counts and findings, fresh-benchmark import/readiness state, the recommended `render-status` command, and next actions. It must not be treated as validation by itself: it does not run missing tests, create delegation evidence, import fresh-session results, finalize, or make an incomplete bundle accepted.
...
Use `implementation enter-task --task <TASK-ID>` as the normal transition command. It records `current_task_id`, preserves the current `READY_TASK_OUTPUT_ID`, and prints `Display Layer: READY Focus`. Use `implementation overview` for read-only progress questions; it must show `Planned Scope`, current TASK, unresolved next TASKs in plan order, and the `--full-plan` audit hint without mutating implementation evidence. Checkpointed, skipped, deferred, failed, or otherwise closed TASKs must not be presented as ordinary next work; if `current_task_id` points at a closed TASK, the overview should say that explicitly so the next agent enters the next unresolved TASK before edits.
...
This audit is required evidence for output-rule or lifecycle-display changes because self-tests only prove constructed fixtures. Transcript audit must fail closed when `Exploration Result`, `Implementation Gate: READY`, or `render-status` appears only in tool stdout, folded transcript output, or one-line summaries while the assistant-visible body lacks the required Display Layer or fixed status fields. It must also fail when tracked delivery actions such as source edits, install, validation, checkpoint/finalize, lease/pre-edit, or output-compliance runs appear in the transcript but the final assistant-visible Closer body is only a casual summary rather than the fixed formal status fields. If a real transcript shows malformed hard output, the TASK must remain incomplete until the output path is fixed and the transcript audit is rerun.
...
If an edit can be represented as a git patch, use `implementation guarded-apply --task <TASK-ID> --patch-file <path>` as the default tracked edit path so the wrapper checks patch paths, active READY/current TASK state, lease coverage, and pre-edit before applying. A successful guarded apply must append `guarded_apply_records`, expose `edit_wrapper_compliance=wrapper-compliant` through `implementation status`, and make `render-status` include the edit wrapper compliance line under `Key Technical Details`. If a tracked edit cannot use `guarded-apply`, Implementer evidence must state the fallback reason and still cite the current `READY_TASK_OUTPUT_ID` and `PRE_EDIT_OK_ID`; fallback edits are not wrapper-compliant evidence. If an edit happened before the guard, record it with `implementation noncompliance --task <TASK-ID> --reason "<reason>" --file <path>`. Open pre-edit noncompliance must be listed by `implementation status`, `verify`, and `render-status`; it cannot be moved only to Key Technical Details or omitted from formal tracked status. When a historical lapse has been dispositioned by user acceptance, superseding corrected execution, or rollback, run `implementation noncompliance-resolve --id <NONCOMPLIANCE_ID> --resolution "<disposition>"`; the event remains in state history with `resolved_at_utc` and no longer blocks unrelated later TASK evidence.
...
The host-hook JSON contracts are intentionally explicit about their limits. `host-hook pre-edit-contract --json` reports `repo_can_intercept_native_tools: false`, and `host-hook final-response-contract --json` reports `repo_can_intercept_final_response: false`. Treat those fields as reviewer evidence that repository tests can verify the contract text, but cannot prove native host interception.
...
For a controlled failed closeout, verify that `finalize --allow-failed-verify` is used only with `--gate-status fail --decision not-accepted`, that accepted/pass closeout still refuses missing pre-close verification, and that `02-report.md` includes `Failed Closeout Evidence` instead of presenting final verification as passed.
