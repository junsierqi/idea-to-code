...
The final formal `Progress` or `Completed` response must map `Changes`, `Completed Items`, `Incomplete Items`, and `Validation Results` back to the same visible TASK/REQ set.

For multi-task work, "same visible TASK/REQ set" means each result bullet maps to the focused execution-level READY excerpt shown before that TASK. The final summary may aggregate TASKs, but it must not introduce unshown or unmapped work.
