...
- READY recovery branch: `implementation recover-ready --task <TASK-ID>` is the stable read-only recovery path after missing or stale READY/Exploration refusals. It prints current IDs, whether editing can continue, concrete problems, and exact recommended commands. It does not mark READY, enter a task, acquire leases, run pre-edit, or replace the user-visible Exploration/READY display requirement.
