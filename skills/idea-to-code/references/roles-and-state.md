...
The following field-by-field rules describe the full display format. The
optional `brief-v1` pair generated by `implementation enter-task --brief` follows
the [brief display contract](verification-and-evidence.md#brief-display-contract)
instead: two visible Planner blocks, current plan-bound scope and the same edit
gates. It is not an arbitrary one-line summary. Review both formats, including
altered and stale brief blocks; a standalone shape check is not state validation.

- Planner output shows `[idea-to-code][Planner/agent] Exploration Result | Bundle: <slug>` for autonomous work or `[idea-to-code][Planner/agent] Confirmation Required | Bundle: <slug>` for confirmation work before READY, with `Display Layer`, `Display Step: 1/2`, the no-edit `Display Boundary`, `Next Layer`, `Required Now`, `Deferred`, `Selected Option`, and `What READY Will Cover` visible to the user. Showing only `EXPLORATION_OUTPUT_ID`, folded command output, or a one-line exploration summary such as `Exploration Result: Required Now = ...` is noncompliant.
- Planner output for an upper-layer profile such as `[idea-to-code/design-to-code][Planner/agent]` must follow the same Exploration/Confirmation contract. The profile prefix is not proof that `exploration render` ran and is not a replacement for the visible `Exploration Result` / `Confirmation Required` block.
- Planner output fails compliance when the required Exploration or READY block appears only in command `tool_stdout` and not in the assistant-visible message body. Use `output-compliance check --kind ready` when reviewing this exact failure mode.
...
- Planner output treats the active bundle state as the execution ledger; it must not create a second independent task list that can diverge from `REQ-*`, `TASK-*`, `IMP-*`, `MB-*`, `current_task_id`, and checkpoint state.
- Planner output shows `[idea-to-code][Planner/agent] Implementation Gate: READY | Bundle: <slug>` and a visible focused TASK/REQ excerpt only after the Exploration Visibility Gate output is current; full READY output is reserved for `--full-plan` audit use. Every current TASK transition needs visible task info for that TASK before edits begin, preferably through `implementation enter-task --task <TASK-ID>` so state records the current TASK. The focused excerpt must include `Display Step: 2/2`, the edit-authorization `Display Boundary`, `Files`, `Execution Details`, `Done Criteria`, `Planned Verification`, `Implementation Granularity`, `Trace Hierarchy`, and the Exploration summary fields; a one-line READY Focus sentence such as `READY Focus TASK-2 / REQ-2: files are ...` is noncompliant.
- Planner output for profile-prefixed tracked work must show `[idea-to-code/<profile-name>][Planner/agent] Implementation Gate: READY | Bundle: <slug>` with the same focused TASK/REQ excerpt. Showing `[idea-to-code/<profile-name>][Planner/agent]` followed by ordinary implementation prose is not READY.
...
- Planner output records a Scope Override Gate before executing non-`Next Action` work while `Incomplete Items` or remaining `MB-*` carryover exists. It must use `scope override`, choose one of `same-ledger-verification`, `same-ledger-repair`, `new-ledger-improvement`, or `accepted-residual`, and preserve the original carryover snapshot in state before any override execution.
...
- Implementer output does not start tracked repository or artifact edits until the visible Exploration Visibility Gate output and READY excerpt have appeared. The executable order is: `implementation enter-task --task <TASK-ID>` records the current TASK, the full Display Layer appears in the assistant-visible main chat, `implementation visible-output record` records that display, `implementation lease acquire --task <TASK-ID> --owner <owner> --file <path>` acquires non-overlapping ownership, and `implementation pre-edit --task <TASK-ID> --file <path>` prints `PRE_EDIT_OK_ID`; only then may tracked edits start. If `00-idea.md` changed after Exploration or READY was generated, the old output IDs are stale and cannot justify edits.
- Implementer output follows Controlled Repair: one current TASK at a time, blocking derived work only when it is needed for the parent TASK, non-blocking derived work recorded as deferred/carryover, and scope-changing derived work routed back to Execution Planning.
- Implementer output for profile-prefixed upper-layer work follows the same gate sequence; the profile label does not authorize edits and does not bypass `visible-output record`, lease, or pre-edit.
...
- Reviewer output flags action-derived response-mode failures: if the turn performed tracked edits, install, verify, validation, checkpoint, finalize, or tracked status delivery, the final response cannot be treated as an ordinary answer even when the initial prompt was ordinary analysis.
...
- Reviewer output runs or cites `output-compliance transcript-audit` when a real transcript is available. For profile-prefixed upper-layer work, the transcript audit must specifically check that `[idea-to-code/<profile-name>]` appears on complete Exploration/READY blocks and final fixed status fields, not only on ordinary progress prose. A review that only says the tool generated READY/render-status, without auditing the assistant-visible transcript body, is incomplete for output-rule changes.
...
- Closer formal tracked status fails compliance if `render-status` appears only in `tool_stdout`, omits any fixed field (`Changes`, `Completed Items`, `Incomplete Items`, `Validation Results`, `Unverified Items`, `Residual Risks`, `Key Technical Details`, or `Next Action`) from the assistant-visible body, puts `Next Action` anywhere except the final formal field, writes `Next Action` content without a `- ` bullet line, drops TASK/REQ mapping from `Changes`, `Completed Items`, `Incomplete Items`, or `Validation Results`, drops IDEA/TASK/REQ mapping when multiple ideas exist in the session ledger, puts `No commit made` under `Incomplete Items`, or hand-writes a formal tracked handoff without first using `render-status` when it is available.
- Reviewer output flags profile-prefixed tracked work that shows only `[idea-to-code/<profile-name>]` without full Exploration/READY blocks before edits or without final fixed `render-status` fields after tracked delivery.
...
The ordinary-answer role check is explicit: a role simulation must fail if it adds READY output, `render-status`, or fixed status fields to explanation-only, naming, clarification, or lightweight commentary messages that stay read-only and do not start tracked delivery actions. It must also fail in the opposite direction if tracked edits, install, validation, checkpoint, finalize, or tracked status delivery occurred but the final response omits `render-status` and the fixed status fields.

Exploration and closeout checks are separate. The Exploration Visibility Gate is a planning/READY Display Layer, while `render-status` is the final tracked handoff Display Layer. Role-sweep findings do not replace `render-status`; a Reviewer should cite both the visible `EXPLORATION_OUTPUT_ID`/`READY_TASK_OUTPUT_ID` path and the final fixed-field handoff when tracked work ran.
