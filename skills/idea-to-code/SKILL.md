...
- **Final response compliance rule**: whenever the assistant-visible final body is available before send, run `output-compliance check --kind formal-status` against the generated `render-status` output and the final body. If the host cannot expose or block the final body before send, disclose that as `host-required` instead of claiming final-body enforcement.
- **Next Action rule**: every formal tracked handoff must include `Next Action`. It must say the next recommended same-scope action, ask for a user decision, or state that no unresolved task remains in this scope. `Next Action` is navigation, not a command that forces the user to continue the same bundle.
