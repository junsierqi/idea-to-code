...
- **Tool-layer edit wrapper rule**: prefer `implementation guarded-apply --task <TASK-ID> --patch-file <path>` when a tracked edit can be represented as a git patch. The wrapper resolves the active bundle, checks patch paths, requires visible Exploration and READY Focus for the current TASK, requires a non-overlapping lease, runs `implementation pre-edit`, captures `PRE_EDIT_OK_ID`, verifies the patch with `git apply --check`, then applies it with `git apply`. Current Codex-native edit tools are still not host-level blocked by this skill; until host pre-edit hooks exist, native-tool bypass remains a `residual risk` and must not be described as impossible.
