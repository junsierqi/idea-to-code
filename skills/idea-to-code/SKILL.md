...
- **Profile caller hard gate**: when an upper-layer skill uses a profile prefix such as `[idea-to-code/design-to-code][Role/source]` for tracked work, it must still invoke or faithfully surface the standard `Exploration Result`, `Implementation Gate: READY`, and final `render-status` fixed-field outputs. The profile prefix is not a lifecycle signal, not edit authorization, and not a substitute for `visible-output record`, lease, pre-edit, installed parity, output compliance, or final fixed status fields.
...
- **Final response compliance rule**: whenever the assistant-visible final body is available before send, run `output-compliance check --kind auto --action <observed-action>...` against the generated `render-status` output and the final body. Pass observed actions such as `edit`, `validation`, `checkpoint`, `finalize`, `install`, `commit`, `render-status`, or `tracked-delivery`; auto mode uses response classification so tracked actions force formal-status validation even when the prompt sounded ordinary. If the host cannot expose or block the final body before send, disclose that as `host-required` instead of claiming final-body enforcement.
- **Next Action rule**: every formal tracked handoff must include `Next Action`. It must say the next recommended same-scope action, ask for a user decision, or state that no unresolved task remains in this scope. Its content must always be a bullet line starting with `- `, even when there is only one item. `Next Action` is navigation, not a command that forces the user to continue the same bundle.
- **Scope Override Gate rule**: when `Incomplete Items` or remaining `MB-*` carryover exists and the user asks to handle non-`Next Action` work such as `Unverified Items`, `Residual Risks`, or another side item first, do not execute it directly. First record `scope override --requested-item ... --classification same-ledger-verification|same-ledger-repair|new-ledger-improvement|accepted-residual --decision ...`. The record must preserve the original incomplete/carryover snapshot. `verify` refuses open Scope Override records; after the override work is executed, linked, moved to a new ledger, accepted as residual, or cancelled, run `scope override-resolve`. `render-status` must keep the original `Incomplete Items` visible and recompute `Next Action`.
...
- **Master backlog range rule**: compressed ranges such as `MB-6..MB-19` or `MB-6 through MB-19` must expand into every stable intermediate `MB-*` item in bundle state before READY/closeout. Do not let a range collapse to only its endpoints; missing intermediate IDs are a continuity failure, not a harmless summary.
- **Remaining backlog / next batch rule**: when a tracked repair is part of a larger MB backlog, every formal tracked result must show the remaining MB list and the next-batch direction until all MB items are `completed` or `covered`. Deferred MB items are not in-scope incomplete work for the current TASK, but they are still remaining backlog and must stay visible. Do not answer a batch closeout with only `Incomplete Items: none` or `Unverified Items: none` if related MB items remain.
...
- **Current TASK entry rule**: before editing files for each TASK/IMP, run or reuse `implementation enter-task --task <TASK-ID>` so the current TASK is machine-recorded and its READY Focus is visible. `implementation show-ready --task <TASK-ID>` is acceptable only as a display fallback when state mutation is impossible; record the reason. A different TASK cannot become current while the previous current TASK is open; close it with checkpoint evidence or `implementation close-task` first.
- **Controlled repair rule**: implementation is a TASK-level repair loop, not free-form fixing. Blocking derived work may interrupt the current TASK only when it is needed to finish or verify that TASK, and the agent must return to the parent TASK afterward. Non-blocking derived work is recorded as deferred/carryover, usually through `MB-*` or follow-up risk notes. Scope-changing derived work stops implementation and returns to Execution Planning before more edits.
...
- `multi-issue-master-backlog`: the user gives several related problems or asks to fix a list such as "1-6". Create stable `MB-*` IDs, register matching REQ/TASK coverage, run `backlog sync`, and show which MB IDs are Required Now, pending, deferred, skipped, blocked, partial, failed, or out of scope before implementation.
...
- <existing pattern evidence, assumption handling, scope boundary, real path/mock policy, regression surface, safety notes, shortcut risk check, and reviewer checks>
Done Criteria:
...
6. **Inspect the current codebase, docs, project governance, and runnable paths** before proposing structure. If project-local rules define authority order, module boundaries, real product paths, validation types, or closeout gates, obey them before generic skill defaults. Every TASK/IMP `Planned Verification` section must name one approved validation type (`real-product-path`, `mock-only`, `fixture-only`, `source-only`, `dom-only`, `manual-inspection`, or `unverified`) before READY; do not defer validation-type selection to checkpoint or finalize.
7. **Decide the next smallest milestone** that creates real progress and keeps the system runnable.
8. **Before editing, fill and print `00-idea.md`.** This is the implementation gate. "Print" means send the task list in a user-visible assistant/commentary message; writing it only to `00-idea.md`, command stdout, a tool result, folded transcript, or internal notes is not enough. During intake and discovery, use the same `TASK-ID` or `IMP-ID` items as the visible task list; placeholder detail values such as `...` are acceptable while `Gate Status` is `DRAFT`. The trace hierarchy is `IDEA-* -> REQ-* -> TASK-* -> optional IMP-*`: `REQ-*` owns user outcomes, `TASK-*` owns execution gates and visible READY scope, and `IMP-*` is used only when a TASK needs lower-level implementation slices. Do not require numeric one-to-one pairing across those IDs, and do not introduce `IMP-*` merely to make a task-only plan look complete. Use this shape even when there is only one task. Before marking the gate `READY`, replace every placeholder with concrete details that are specific enough to verify but not so fine-grained that they describe every line edit:
   ```text
   ...
   Mark the gate ready only after every TASK has concrete, non-placeholder `Files`, `Execution Details`, `Implementation Quality Contract`, `Done Criteria`, and `Planned Verification`:
   ```bash
   ...
   When this command succeeds, it prints the user-visible `[idea-to-code][Planner/agent] Implementation Gate: READY` message with every TASK, `Files`, `Execution Details`, `Implementation Quality Contract`, `Done Criteria`, `Planned Verification`, and `READY_TASK_OUTPUT_ID`. The quality contract is the TASK execution boundary: evidence must come from existing patterns or validation, assumptions stay explicit, scope and mock/real-path choices are bounded, shortcut risks are named, and Reviewer evidence must check those claims. It is not a second exploration loop and must not add unrelated work. When another skill uses idea-to-code as a lifecycle foundation, it may pass `--profile <profile-name>` so the READY message starts with `[idea-to-code/<profile-name>][Planner/agent]`. The profile is display-only and must not be treated as bundle state, requirement scope, role evidence, or lifecycle policy. Do not infer trust, ownership, permissions, or scope from a profile name; it is only a user-visible label.
   ...
   ```
   `enter-task` records `current_task_id`, preserves the existing `READY_TASK_OUTPUT_ID`, and prints `Display Layer: READY Focus`. Use it before edits for TASK-1 and again before every TASK transition. A TASK transition is refused while the current TASK is open. Use `checkpoint` to close a verified/partial/failed implementation result with evidence, or `implementation close-task` to close skipped, blocked, deferred, partial, failed, or replan outcomes that do not have implementation evidence. Use `implementation overview` when the user asks where the work stands; it prints `Planned Scope`, current TASK, next TASKs, and the `--full-plan` audit hint without changing state.
   ...
   ```
   Open pre-edit noncompliance appears in `implementation status`, `verify`, and `render-status`; accepted closeout must not bury it in technical details. If the lapse is later dispositioned by user acceptance, superseding corrected execution, or rollback, run `implementation noncompliance-resolve --id <NONCOMPLIANCE_ID> --resolution "<disposition>"` so history remains visible but the open blocker no longer prevents unrelated later TASK evidence.
...
9. **Record Planner evidence before implementation**:
   Follow the Role Evidence Checklist in `references/roles-and-state.md` before recording any role evidence. Role evidence recording is an ordered mutation sequence; do not run `role record` commands in parallel. Use `role explain --role <role>` as a read-only helper when you need the checklist in JSON form, or after `role record` rejects evidence. Use `role draft --role <role> --covers <REQ-IDS> --task <TASK-ID>` when the evidence wording is uncertain; it generates a read-only draft with current lifecycle IDs and blockers. Both helpers are read-only; `role draft` does not change state and does not replace `role record`.
   ```bash
...
12. **Review the result before checkpoint/finalize.** Make a separate Reviewer pass over scope, diffs, architecture boundaries, acceptance matrix coverage, validation strength, the current TASK `Implementation Quality Contract`, shortcut risk, evidence discipline, unverified items, and residual risks. Record Reviewer evidence. Do not claim an independent reviewer ran unless one actually did.
...
Run the repo-enforced parity command before claiming installed runtime behavior:

```bash
python "$HOME/.codex/skills/idea-to-code/scripts/idea_to_code_bundle.py" install-parity check \
  --source <source-skill-dir> \
  --target "$CODEX_HOME/skills/idea-to-code"
```

If installed focused tests or source/installed SHA256 parity have not passed, do not claim the latest skill code is installed and verified. Put the missing install evidence under `Unverified Items` or the relevant unfinished TASK/REQ instead. Upper-layer or wrapper skills that build on idea-to-code must either call the same installed base skill and parity gate, or disclose that their installed base-skill parity is unverified.
...
python "$HOME/.codex/skills/idea-to-code/scripts/idea_to_code_bundle.py" fresh-benchmark verify-import --root "$(pwd)" --slug <slug>
...
`fresh-benchmark verify-import` is the repo-enforced evidence-quality gate for imported results. It checks that the import has `external-status completed`, an existing raw output artifact, a non-empty score JSON object with scoring detail, and required raw-output protocol markers such as `[idea-to-code]`, `/fresh-agent]`, `Exploration Result`, `Implementation Gate: READY`, `EXPLORATION_OUTPUT_ID`, and `READY_TASK_OUTPUT_ID`. A passing `verify-import` does not prove host-level runtime isolation by itself; it only proves the imported artifact satisfies the repository's evidence-admission rules.
...
For tracked work, a profile-aware upper-layer skill must call the same lifecycle outputs with `--profile <profile-name>` or faithfully surface their exact fields in the assistant-visible body: `exploration render`, `implementation ready` / `implementation show-ready`, `implementation visible-output record`, lease, pre-edit, validation/review evidence, and final `render-status`. A message that only starts with `[idea-to-code/<profile-name>][Planner/agent]` or `[idea-to-code/<profile-name>][Closer/agent]` but omits the required Exploration/READY blocks or final fixed status fields is noncompliant.

Formal status output compliance treats `[idea-to-code/<profile-name>][Closer/agent] Status: ...` as the same idea-to-code fixed-field contract as `[idea-to-code][Closer/agent] Status: ...`. The profile prefix does not relax fixed field order, TASK/REQ mapping, output ID, `Unverified Items`, or `Residual Risks` rules. A profile-prefixed natural-language completion summary after tracked work is not a compliant final result unless it preserves the `render-status` fixed fields in the assistant-visible final body.

Transcript audit must also catch the case where the turn clearly performed tracked delivery actions but never showed a formal Closer status at all. Source edits, install, validation, checkpoint/finalize, lease/pre-edit, or output-compliance runs followed by a casual completion sentence are noncompliant even when no `render-status` helper output is present in the transcript.
...
For formal tracked `Progress`, validation, install, or status responses, every `Changes`, `Completed Items`, `Incomplete Items`, and `Validation Results` bullet must name the relevant concrete `TASK-N` and `REQ-N` IDs when the work entered bundle accounting. Placeholder labels such as `TASK-*` or `REQ-*` are not compliant result mapping in a final assistant-visible body. Those IDs must map back to the READY TASK list or focused READY excerpt surfaced in a normal assistant message before implementation. Do not report substantial check/install/validation work as tracked Progress unless that work is represented by a READY TASK. If a response cannot map to a TASK/REQ, answer naturally as an ordinary explanation or first update the bundle plan.
...
The helper prints the fixed field skeleton with the idea-to-code role/source prefix, TASK/REQ mapping placeholders, `EXPLORATION_OUTPUT_ID`, `READY_TASK_OUTPUT_ID`, `No commit made` under Key Technical Details by default, and final `Next Action`. When milestone, IDEA ledger, backlog, session, scope, delegation, or noncompliance evidence exists, the helper should surface that evidence directly and keep placeholders only where evidence is genuinely missing. Formal tracked status MUST use render-status generated fields when the helper is available: edit the skeleton with actual evidence before sending it; do not remove fixed fields, rename them, reorder them, or hand-invent them; do not drop concrete `EXPLORATION_OUTPUT_ID` or `READY_TASK_OUTPUT_ID` from `Key Technical Details`; do not leave `<EXPLORATION_OUTPUT_ID>` or `<READY_TASK_OUTPUT_ID>` placeholders in a final body; do not drop TASK/REQ mapping from `Changes`, `Completed Items`, `Incomplete Items`, or `Validation Results`; do not drop IDEA/TASK/REQ mapping when multiple ideas exist in the session ledger; do not write free-floating `Residual Risks`; every non-`none` residual risk bullet must name concrete `TASK-N`, `REQ-N`, `MB-N`, or `IDEA-N` scope; `Next Action` must be the final formal field, must use a `- ` bullet line, and must name the next recommended action or say no unresolved task remains; and do not move no-commit state into `Incomplete Items`. Before sending, prefer `output-compliance check --kind auto --action ...` over manually selecting `formal-status`; auto mode makes tracked delivery actions override prompt wording. Do not use fixed fields for ordinary untracked answers.

`READY_TASK_OUTPUT_ID` is the latest READY output. READY refresh history is retained separately as bounded `ready_task_output_history` in `state.json` and surfaced by `implementation status` plus `render-status` as `READY_TASK_OUTPUT_HISTORY`. Use the history when debugging repeated READY/show-ready refreshes; do not replace latest-ID checks with historical IDs for pre-edit or role evidence.
